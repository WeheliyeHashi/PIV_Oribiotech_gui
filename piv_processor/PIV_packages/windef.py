...
def func(args, settings, save_path):
    ...
    tools.save(x, y, u, v, mask,
                os.path.join(save_path, "field_A%06d.txt" % counter),
                delimiter="\t")
...
def piv(settings):
    ...
    print(settings.n_cpus)
    task.run(func=func, n_cpus=settings.n_cpus, settings=settings, save_path=save_path)
...
def multipass_img_deform(
    ...
    #u, v, mask = validation.typical_validation(u, v, s2n, settings)
    u, v, mask = validation.local_median_val(u,v,1.2,1.2,1)
