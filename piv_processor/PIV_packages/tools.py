...
class Multiprocesser:
    def __init__(self, data_dir, pattern_a, pattern_b=None):
        ...
        self.files_a = pattern_a
        self.files_b = pattern_b
    ...
    def run(self, func, n_cpus=1, settings=None, save_path=None):
        ...
        func_with_args = partial(func, settings=settings, save_path=save_path)
        print("Number of CPUs: ", n_cpus)
        if n_cpus > 1:
            
            pool = multiprocessing.Pool(processes=n_cpus)
            res = pool.map(func_with_args, image_pairs)
        else:
            for image_pair in image_pairs:
                func_with_args(image_pair)
